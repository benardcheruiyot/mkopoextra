...
const maskedPhoneRegex = /^0(?:7(?:[0-2]\d|4[0-3]|45|46|48|5[7-9]|6[89]|9\d)|11[0-9])\*{4}\d{2}$/;
...
const Loan = () => {
  ...
  const maskedRecentLoans = recentLoans.filter((entry) => maskedPhoneRegex.test(entry.phone));
  const carouselLoans = maskedRecentLoans.length
    ? maskedRecentLoans
    : recentLoans;
  ...
  useEffect(() => {
    document.title = 'Loan Options | LendHub';
  }, []);
  ...
  const handleApply = async () => {
    ...
    try {
        ...
        html: `
          <div class="payment-modal-content">
            ...
            <p class="payment-summary-note">
              A secure payment request will be sent to ${user.phone_number}.
            </p>
          </div>
        `,
        customClass: {
      ...
      Swal.fire({
        ...
        html: `
          <div class="payment-modal-content">
            ...
            <p class="payment-instruction">Confirm the payment request on your phone to approve payment.</p>
            <div class="payment-status-pill">Amount: ${formatCurrency(selectedLoan.fee)}</div>
            <p class="payment-progress-note">Waiting for payment confirmation...</p>
            <p class="payment-progress-sub" id="paymentAttemptHint">This usually takes less than 60 seconds.</p>
          </div>
        `,
        customClass: {
          popup: 'payment-modal',
      ...
      // Initiate payment
      const result = await loanService.initiatePayment(
        user.phone_number,
        selectedLoan.fee,
        selectedLoan.amount,
        selectedLoan.days
        ...
        if (Swal.isVisible()) {
          ...
          Swal.update({
            html: `
              <div class="payment-modal-content">
                <div class="payment-spinner" aria-hidden="true"></div>
                <p class="payment-instruction">Confirm the payment request on your phone.</p>
                <div class="payment-status-pill">Amount: ${formatCurrency(selectedLoan.fee)}</div>
                <p class="payment-progress-note">Waiting for payment confirmation...</p>
                <p class="payment-progress-sub">Checking status... ${remainingSeconds}s remaining</p>
              </div>
            `,
          });
        }
        ...
        try {
          ...
          if (statusResult.success) {
            ...
            Swal.fire({
              ...
              html: `
                <div class="payment-modal-content">
                  ...
                  <p class="payment-instruction">Payment received successfully. Your loan will be processed and disbursed within 48 hours.</p>
                </div>
              `,
              customClass: {
                  popup: 'payment-modal',
            ...
            if (!autoRetryUsedRef.current && hasAgentStoreMismatch) {
              ...
              try {
                const retryResult = await loanService.initiatePayment(
                  user.phone_number,
                  selectedLoan.fee,
                  selectedLoan.amount,
                  selectedLoan.days
                  ...
                  if (Swal.isVisible()) {
                    Swal.update({
                      html: `
                        <div class="payment-modal-content">
                          <div class="payment-spinner" aria-hidden="true"></div>
                          <p class="payment-instruction">We are sending a new payment request. Confirm it on your phone.</p>
                          <div class="payment-status-pill">Amount: ${formatCurrency(selectedLoan.fee)}</div>
                          <p class="payment-progress-note">Retry request sent successfully.</p>
                          <p class="payment-progress-sub">Waiting for payment confirmation...</p>
                        </div>
                      `,
                    });
                  }
                ...
                console.error('Automatic payment retry failed:', retryError);
              }
            }
      ...
      <div className="apply-container">
        <div className="loan-content card">
          <div className="loan-topbar">
            <div className="loan-topbar-copy">
              ...
              <p className="apply-subtitle">
                Pick one offer to continue. You will confirm details before any payment request is sent.
              </p>
            </div>
            ...
            <div className="loan-topbar-summary" aria-live="polite">
              <div className="loan-trust-card" aria-label="Loan trust highlights">
                ...
                <div className="loan-trust-points">
                  ...
                  <span>Secure payment prompt</span>
          ...
          <div className="fee-note">
            <p>
              💡 Your processing fee is paid via the selected payment provider. Once payment is confirmed, we move you to the processing
