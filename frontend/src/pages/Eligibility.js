...
const phoneRegex = /^254(?:7(?:[0-2]\d|4[0-3]|45|46|48|5[7-9]|6[89]|9\d)|11[0-9]|14[0-9])\d{6}$/;
...
function normalizePhone(raw) {
  ...
  // If already starts with country code 254, return as-is
  if (p.startsWith('254')) return p;
  // Local formats: 0XXXXXXXXX or 7XXXXXXXX
  if (p.startsWith('0') && p.length === 10) return '254' + p.slice(1);
  if (p.length === 9 && p.startsWith('7')) return '254' + p;
  if (p.length === 10 && p.startsWith('1')) return '254' + p; // handle rare leading 1 cases
  return p;
}
...
const Eligibility = () => {
  ...
  const [fieldState, setFieldState] = useState({
    ...
    phone_number: { state: null, msg: 'Enter your mobile number, e.g. 254712345678 or 0712 345 678' },
  ...
  useEffect(() => {
    document.title = 'Loan Eligibility | LendHub';
  }, []);
  ...
  const validatePhone = (raw) => {
    if (!raw) return { state: null, msg: 'Enter your mobile number, e.g. 254712345678 or 0712 345 678' };
    const v = normalizePhone(raw);
    if (v.length < 12) return { state: 'invalid', msg: 'Too short — expected full Kenyan number in international or local format' };
    if (!phoneRegex.test(v)) return { state: 'invalid', msg: 'Not a valid mobile number — expected Kenyan format like 254712345678' };
    return { state: 'valid', msg: '\u2713 Valid mobile number' };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!phoneRegex.test(phone)) {
      return Swal.fire({
        icon: 'error', title: 'Valid Mobile Number Required',
        html: '<p>Please enter a valid Kenyan mobile number.</p><p style="margin-top:8px;font-size:0.9rem;color:#555;">Examples:</p><ul style="text-align:left;margin-top:4px;font-size:0.9rem;"><li>Local: 0712 345 678</li><li>International (E.164): 254712345678</li></ul>',
